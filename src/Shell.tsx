  ...
  implements ShellFunctions
{
  ...
  componentDidUpdate(prevProps: Props) {
    ...
    if (this.props.attachmentUrl && !this.state) {
      ...
      }).then((attachmentQrCode: string) =>
        this.setState({ attachmentQrCode })
      );
    }
  }
  ...
  private onChangeFile() {
    this.props.sendFiles(
      ...
      location.hash.includes("#feedbot-direct-upload")
    );
      ...
      if (
        ...
        reader.onload = (function (file) {
          return async function (e: any) {
            ...
            var base64String =
              "data:" + file.type + ";base64," + window.btoa(binaryData);
            const body = JSON.stringify([
              { data: base64String, fileName: file.name },
            ]);
            try {
  ...
  render() {
    ...
    const sendButtonClassName = classList(
      "wc-send",
      showMicButton ||
        (this.props.showUploadButton && this.props.disableInput && "hidden")
    );
    ...
    const localAndQrAttachment = [
      <StyledDropZone
        key="a"
        label={this.props.strings.attachmentDropArea}
        onDrop={(file: any) => this.addFile(file)}
      />,
      <div key="b" className="attachment-wrapper">
        <span className="attachment-url">
          {this.props.strings.attachmentInfo}
        </span>
        <a
          href="#"
          onClick={() =>
            alert(
              this.props.strings.attachmentAlert +
                "\n\n" +
                this.props.attachmentUrl
            )
          }
        >
          <img src={this.state && this.state.attachmentQrCode} />
        </a>
      </div>,
    ];
    ...
    const qrOnlyAttachment = [
      <div key="b" className="attachment-wrapper qr-only">
        <span className="attachment-url">
          ...
          <br />
          <span>{this.props.attachmentUrl}</span>
        </span>
        <a
          href="#"
          onClick={() =>
    ...
    return (
      <div className={className}>
        {this.props.showUploadButton &&
          (this.props.uploadUsingQrCodeOnly
            ? qrOnlyAttachment
            : localAndQrAttachment)}
        {this.props.showUploadButton && (
          <input
            id="wc-upload-input"
            tabIndex={-1}
...
export const Shell = connect(
  ...
  (stateProps: any, dispatchProps: any, ownProps: any): Props => ({
    ...
    sendFiles: (files: FileList, isDirectUpload: boolean) =>
      dispatchProps.sendFiles(
        files,
        stateProps.user,
        stateProps.locale,
        isDirectUpload
