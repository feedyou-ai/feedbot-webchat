  ...
  switch (action.type) {
    ...
    case "Toggle_Upload_Button":
      return {
        ...
        uploadUsingQrCodeOnly: action.hasOwnProperty("uploadUsingQrCodeOnly")
          ? action.uploadUsingQrCodeOnly
          : state.uploadUsingQrCodeOnly,
      };
  ...
  action$.ofType("Receive_Message").map(
    (action) =>
      ({
        ...
        attachmentUrl:
          action.activity.channelData &&
          action.activity.channelData.attachmentUrl,
      } as FormatAction)
  );
    ...
    .map(
      (action) =>
        ({
          type: "Toggle_Disable_Input",
          disableInput:
            action.activity.inputHint === "ignoringInput" ||
            action.activity.inputHint === "expectingUpload",
        } as FormatAction)
    );
